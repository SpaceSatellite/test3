...
for i in column_names:
    ...
    for k in column_names[a:b]:
        ss_2 = f'create table if not exists {i}_{k}(id_open, id_close, open, high, low, close, profit, clear_profit, profit_p, clear_profit_p, lowest_delta, lowest_delta_p, lost_income, lost_income_p)'
        cur_2.execute(ss_2)
        con_2.commit()
        ...
        for l in q_part:
            ...
            if diff != flag:
                ...
                if deal_open == 0:
                    ...
                    deal_lowest_delta = deal_low - deal_open if flag == -1 else deal_high - deal_open
                    deal_lowest_delta_p = (deal_lowest_delta/deal_open)*100 
                    deal_lost_income = deal_high - deal_close if flag == -1 else deal_low - deal_close
